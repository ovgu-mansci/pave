...
public class GenerateTestData  extends TestCase {
	...
	protected static Route generateTestRoute() {
		...
		route.getRouteElements().add(SolutionHandler.createRequestActivityRouteElement(request.getFirstActivity()));
		...
		route.getRouteElements()
				.add(SolutionHandler.createRequestActivityRouteElement(request.getSecondActivity()));
	...
	protected static Config generateTestConfig() {
		...
		config.getAlgorithm().getLns().getAlns().setMaxCalculationTime(algorithm_Lns_MaxCalculationTime);
		config.getAlgorithm().getLns().getAlns().setMaxInsertionIterations(algorithm_Lns_MaxIterations);
		...
		assertEquals(algorithm_Lns_MaxCalculationTime, config.getAlgorithm().getLns().getAlns().getMaxCalculationTime());
		assertEquals(algorithm_Lns_MaxIterations, config.getAlgorithm().getLns().getAlns().getMaxInsertionIterations());
	...
	protected static Input generateTestInput() {
		...
		for (Request request : requests) {
			request.setRequestTime(99);
			request.getFirstActivity().setEarliestArrival(999);
			request.getFirstActivity().setServiceDuration(9);
			request.getFirstActivity().setLatestArrival(99999);
			request.getSecondActivity().setEarliestArrival(111);
			request.getSecondActivity().setServiceDuration(1);
			request.getSecondActivity().setLatestArrival(11111);
		}
	...
	protected static Solution generateTestSolution(Input input, boolean useVehicleType) {
		...
		for (Route route : routes) {
			route.getRouteElements().add(
					SolutionHandler.createRequestActivityRouteElement(input.getRequests().getNew().get(2).getFirstActivity()));
			...
			route.getRouteElements().add(SolutionHandler
					.createRequestActivityRouteElement(input.getRequests().getNew().get(2).getSecondActivity()));
