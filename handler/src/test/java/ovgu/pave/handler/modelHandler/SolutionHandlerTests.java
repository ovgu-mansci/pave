...
public class SolutionHandlerTests extends TestCase {
	...
	public void test_getRouteElementIndex() {
		...
		RouteElement routeElement1 = route.getElement(index1);
		RouteElement routeElement2 = route.getElement(index2);
	...
	public void insert_Activity_RouteElement_Test(int index, boolean useRouteElement) {
		...
		RequestActivity activity = InputHandler.createRequestActivity(SecondRequestActivity.class);
	...
	public void test_insertRequest() {
		...
		int indexFirst1 = 1;
		int currentIndexSecond1 = 2;
		int indexFirst2 = 0;
		int currentIndexSecond2 = 3;

		SolutionHandler.insertRequest(route1, request1, indexFirst1, currentIndexSecond1);
		SolutionHandler.insertRequest(route1, request2, indexFirst2, currentIndexSecond2);

		assertEquals(route1.getRouteElements().size(), route2.getRouteElements().size() + 3);
		for (int i = 0; i < route1.getRouteElements().size(); i++) {
			if (i == 0) // indexFirst2
				assertTrue(EcoreUtil.equals(request2.getFirstActivity(),
						((RequestActivityRouteElement) route1.getRouteElements().get(i)).getRequestActivity()));
			else if (i == 1)
				assertTrue(EcoreUtil.equals(route1.getRouteElements().get(i), route2.getRouteElements().get(i - 1)));
			else if (i == 2) // indexFirst1 + 1 (because of origin 2 is before)
				assertTrue(EcoreUtil.equals(request1.getFirstActivity(),
						((RequestActivityRouteElement) route1.getRouteElements().get(i)).getRequestActivity()));
			else if (i == 3)
				assertTrue(EcoreUtil.equals(route1.getRouteElements().get(i), route2.getRouteElements().get(i - 2)));
			else if (i == 4) // currentIndexSecond1 + 1 (First1) + 1 (First2)
				assertTrue(EcoreUtil.equals(request1.getSecondActivity(),
						((RequestActivityRouteElement) route1.getRouteElements().get(i)).getRequestActivity()));
			else if (i > 3)
	...
	public void test_getRequestActivity_getRequest_getLocation_getLatitude_getLongitude_isWhichRouteElement() {
		...
		RequestActivity ra1 = request1.getFirstActivity();
