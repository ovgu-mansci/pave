...
public class NetworkHandler {
	...
	public <T1, T2> long getTravelDuration(T1 from, T2 to) {
		...
		if ((SolutionHandler.isSupportRouteElement(from) && ((SupportRouteElement) from).getLocation() == null)
				|| (SolutionHandler.isSupportRouteElement(to) && ((SupportRouteElement) to).getLocation() == null))
			return 0l;

		return (long) (getEdge(from, to).getDuration()
				* Handler.getInput().getConfig().getExperiment().getFactorOnTravelTime());
	}
	...
	public <T1, T2, T3> long getInsertionDuration(T1 from, T2 over, T3 to) {
		long insertionCosts = getTravelDuration(from, over) + getTravelDuration(over, to) - getTravelDuration(from, to);
		if(insertionCosts < -100) System.out.println("ERROR: Negative insertion costs");
		return insertionCosts;
	}

	public <T1, T2, T3, T4> long getInsertionDurationIfIndexesEqual(T1 from, T2 overFirst, T3 overSecond, T4 to) {
		long insertionCosts = getTravelDuration(from, overFirst) + getTravelDuration(overFirst, overSecond)
				+ getTravelDuration(overSecond, to) - getTravelDuration(from, to);
		if(insertionCosts < -100) System.out.println("ERROR: Negative insertion costs by " + insertionCosts + " ms.");
		return insertionCosts;
	}
	...
	public <T1, T2> Edge getEdge(T1 from, T2 to) {
		...
		if (fromLocation != null && toLocation != null) {
			...
			if (fromLat == -1 || fromLon == -1 || toLat == -1 || toLon == -1)
				return null;
			else if (fromLat == 0 || fromLon == 0 || toLat == 0 || toLon == 0)
				return InputHandler.createEdge(Handler.getInput().getLocations().get(0),
						Handler.getInput().getLocations().get(0));
			else
	...
	private synchronized Edge getEdge(double fromLat, double fromLon, double toLat, double toLon) {
	...
	private static <T> boolean isWayPoint(T object) {
		return object instanceof WayPoint;
	}
	...
	private static <T> double getLatitude(T object) {
		...
		else if(isWayPoint(object))
			return  ((WayPoint) object).getLat();
		else if (SolutionHandler.isRequestActivity(object))
